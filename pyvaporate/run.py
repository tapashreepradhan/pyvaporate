...
def yaml_run(config_file):
    ...
    if "%" in n_events_per_step:
        ...
        n_events_per_step = 0
    while step_number * SETUP["events_per_step"] < SETUP["total_events"]:
        ...
        if step_number == 0:
            ...
            if n_events_total == np.inf:
                SETUP["total_events"] = math.ceil(total_percent * n_atoms)
            if n_events_per_step == 0:
                SETUP["events_per_step"] = math.ceil(step_percent * n_atoms)
        else:
